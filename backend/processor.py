...
以下の複数の情報をもとに、初心者コンサルタント向けの教育コンテンツを一括で作成してください。
情報源が動画トランスクリプトか、ニュース記事かによって適切なコンポーネントのみを生成してください。
（ニュースであれば「News」と「Quiz」を出力し、深い運用系の動画であれば「Manual」や「Case Study」を出力するなど）

以下のJSON配列形式で、作成可能なコンポーネントのみを含めて出力してください（不要なキーは省略可能）。
Markdownコードブロック(```json)は含めず、純粋なJSON配列のみを出力してください。

【重要：テーマ分類】
manual, quiz, case_study を生成する場合、必ず以下のテーマのいずれか1つを正確に `theme` キーに設定してください。記述が少しでも違うとエラーになります。
